              ...
              <tr v-for="order in submittedOrders" :key="order.id">
                ...
                <td class="col-lead">
                  <span class="badge info">
                    {{ daysUntil(order.expected_delivery) === 0
                      ? 'Arriving today'
                      : `Arrives in ${daysUntil(order.expected_delivery)} days` }}
                  </span>
                </td>
...
export default {
  ...
  setup() {
    ...
    // Returns days from now until an ISO date string; null if date is invalid
    const daysUntil = (iso) => {
